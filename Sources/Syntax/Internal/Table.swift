...
extension Table {
    fileprivate typealias Row = [FormattedText]
    fileprivate typealias Cell = FormattedText

    fileprivate static let delimiters: Set<Character> = ["|", "\n"]
    fileprivate static let allowedHeaderCharacters: Set<Character> = ["-", ":"]

    fileprivate enum ColumnAlignment {
        case none
        case left
        case center
        case right
    ...
    fileprivate mutating func formHeaderAndColumnAlignmentsIfNeeded() {
    ...
    fileprivate func parseColumnAlignment(from text: String) -> ColumnAlignment {
    ...
    fileprivate func plainText(forRow row: Row) -> String {
        var text = ""
...
extension Reader {
    fileprivate mutating func readTableRow() throws -> Table.Row {
    ...
    fileprivate mutating func readTableDelimiter() throws {
        try read("|")
        discardWhitespaces()
    }
}
