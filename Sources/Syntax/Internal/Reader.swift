...
extension Reader {
    ...
    @discardableResult
    mutating func read(
        until character: Character,
        required: Bool = true,
        allowWhitespace: Bool = true,
        allowLineBreaks: Bool = false,
        balanceAgainst balancingCharacter: Character? = nil
    ) throws -> Substring {
    ...
    @discardableResult
    mutating func readCharacters(
        matching keyPath: KeyPath<Character, Bool>,
        max maxCount: Int = Int.max
    ) throws -> Substring {
        let startIndex = currentIndex
        var count = 0
...
extension Reader {
    fileprivate func lookBehindAtPreviousCharacter() -> Character? {
    ...
    fileprivate func lookAheadAtNextCharacter() -> Character? {
